...
## 2026-09-20 · After M5: stacked controls on a real shop (nike.com), and the last shop wording

**Start 10:35 · End 10:54** (two fixes Sean asked for after his run on nike.com; both trial fixes were already spent, so this is his call, not the trial's.)

**What Sean saw.** On Nike's men's shoes listing, "click on the Air Jordan 12" ended in "I couldn't do that here." His question: even without a vision model, how does it not see that?

**It did see it.** "I couldn't do that here." is the executor refusing, not Jev failing to find (that would be "I can't find that on this page."). Measured on the live page: one Nike product card is **five rows for one product**. A link stretched over the whole card (`position: absolute`, its text is the product's name); a second link to the same address around the picture (named by `aria-label`); and three `div[role=link]` without an address underneath: the name again, "Men's Shoes", and "$215". The executor's last check before a click asks whether the thing under the target's centre is the target. For the stretched link the picture inside the *other* link is there; for the name, the stretched link is there. So two of the three rows that carry the product's name were refused as "covered by something else", and Jev had picked one of them. The check exists to stop clicks on things behind pop-ups; here it was stopping a click on the same product by another name.

**The fix** (`agent/execute.ts`, `sameControl`): when what is on top is, or is inside, a link or button that is the same control, it is not a cover, and **that one is pressed**, because it is what a person's click at that spot would press. Same control means: both are real links to the same address; or, where addresses cannot tell (one of them has none), the same accessible name.

**What the review made of my first version** (3 lenses, one sceptic per finding, 16 agents; 12 findings confirmed, 1 refuted, about 9 distinct). My first rule was "same address or same name", and it was too loose in ways that mattered, because every rule upstream (deny-list, confirm card, submit rule, blockers) had judged the *target*, and now something else was being pressed. All reproduced by the sceptics against the real module, all fixed, each with a test:

- `href="#"`, `href=""` and `javascript:` links all resolve to the same address, so "Show more" under "Accept all cookies" counted as the same place and the cookie link would have been pressed. An address now counts only when it goes somewhere.
- A same-named button inside a pop-up or a sticky bar ("Sign up" over a "Sign up" link) was accepted, which skipped the blocker logic and pressed the pop-up's own accepting button. A twin is never looked for across the edge of a dialog or a fixed or sticky container; the loop gets its `coveredBy` as before.
- Two real links with the same name and different addresses (a carousel of "Shop now") are different controls: where they go decides, not what they are called.
- A disabled twin, another form's same-named submit button, and typing or choosing through a twin (only a click may go through) are refused.
- With a twin on top the target is still scrolled to the middle first, in case the twin was only a sticky bar passing over it.
- The extension bundle was stale, as it is after every change: rebuilt.

**The shop wording.** The memory panel's empty text said "I remember answers such as your size, never tastes like brand or colour." Now: "Nothing saved for this site yet. When I have to ask you something only you can know, I remember the answer and do not ask again on this site." The sweep found nothing else shop-specific that a user sees or hears on an arbitrary site; "No matches with these filters." was challenged and kept (SPEC wording, and not shop language). The dev-only states gallery still shows the store's own examples. My first rewrite promised two things the code does not do, and the review caught both: answers are kept **per site** (a size saved on nike.com is not used on another shop), and **every** answer to a question the agent itself asks on a task is saved, tastes included; only "Narrow by" answers are filtered by the personal Noul. That second point is what the SPEC says under "Remember" (section 9. Asking, answers, and memory), so the code stays and the sentence promises less.

**Jev wording:** untouched.
