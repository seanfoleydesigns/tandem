...
// Structured outputs want every property present, so "not stated" is null rather than missing.
...
const PARSE_SYSTEM = `You turn a shopper's spoken request into search constraints for a shop page.
Return only what the request states. Use null for anything it does not state.
...
- When the page lists categories or filter options, use the page's own word if one matches what the shopper said (for example "trainers" becomes "Sneakers").
- search_query: only when the shopper asks to search for specific words.
- visual_prefs: looks that no filter captures, such as "no big logo". Usually an empty list.
- A request that only adjusts the current results ("only the ones under a hundred and fifty") states just that adjustment.`;

const VERIFY_SYSTEM = `You check whether a shop page now shows what the shopper asked for, and you write one short sentence that will be spoken aloud.
...
- ok is false only when the page plainly does not match the goal: the wrong kind of product, a colour or other named filter not applied, or no results.
- A price limit is never applied as a filter on the page. The assistant dims the results outside the limit on the shopper's screen instead, and that is the intended outcome. So results outside the limit, or a price filter that is not set, are never an issue and never make ok false. The one exception: when "within_price" is 0, ok is false, because nothing fits the price.
- issues: short phrases; an empty list when ok is true.
- spoken: at most ${SPOKEN_MAX_WORDS} words, plain and friendly, no lists. Say how many results there are and, when there is a price limit, how many are within it. Example: "Nine white sneakers in your size, six under a hundred dollars." When ok is false, say what is off instead.`;
...
export async function parseGoal(req: ParseRequest): Promise<ParseResponse> {
  ...
  if (data) {
    if (data.category) constraints.category = data.category;
    if (data.colour) constraints.colour = data.colour;
...
export async function verifyAndSummarise(req: VerifyRequest): Promise<VerifyResponse> {
  ...
  const spoken = words.length <= SPOKEN_MAX_WORDS ? data.spoken.trim() : '';
